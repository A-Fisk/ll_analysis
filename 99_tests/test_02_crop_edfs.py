...
spec = importlib.util.spec_from_file_location(
    "crop_edfs",
    os.path.join(
        os.path.dirname(__file__), "..", "02_preprocessing", "02_crop_edfs.py"
    ),
)
...
class TestCropEDFs:
    ...
    def test_crop_edf_to_24_hours_normal_case(
        self, mock_read_edf, mock_write_edf, temp_directory
    ):
        ...
        write_kwargs = (
            mock_write_edf.call_args[1] if mock_write_edf.call_args[1] else {}
        )
    ...
    @patch.object(crop_edfs.pyedflib.highlevel, "write_edf")
    @patch.object(crop_edfs.pyedflib.highlevel, "read_edf")
    def test_crop_edf_to_24_hours_short_data(
        self, mock_read_edf, mock_write_edf, temp_directory
    ):
    ...
    def test_change_to_git_root_success(self, mock_chdir, mock_check_output):
        ...
        mock_check_output.assert_called_once_with(
            ["git", "rev-parse", "--show-toplevel"], stderr=crop_edfs.subprocess.DEVNULL
        )
    ...
    def test_change_to_git_root_not_git_repo(self, mock_chdir, mock_check_output):
        ...
        mock_check_output.side_effect = crop_edfs.subprocess.CalledProcessError(
            1, "git"
        )
    ...
    def test_main_function_flow(
        self, mock_process_all, mock_discover, mock_setup_dirs, mock_setup_env
    ):
        ...
        mock_process_all.assert_called_once_with(
            mock_file_list, mock_input_dir, mock_output_dir
        )
...
class TestIntegration:
    ...
    def test_full_pipeline_integration(
        self, mock_write_edf, mock_read_edf, temp_directory
    ):
    ...
    @patch.object(crop_edfs.pyedflib.highlevel, "read_edf")
    @patch.object(crop_edfs.pyedflib.highlevel, "write_edf")
    def test_edge_case_high_sample_rate(
        self, mock_write_edf, mock_read_edf, temp_directory
    ):
