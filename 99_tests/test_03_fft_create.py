...
spec = importlib.util.spec_from_file_location(
    "fft_create",
    os.path.join(
        os.path.dirname(__file__), "..", "02_preprocessing", "03_fft_create.py"
    ),
)
...
class TestFFTProcessing:
    ...
    def test_setup_directories(self, temp_directory):
        ...
        with patch.object(fft_create, "Path") as mock_path:
    ...
    @patch("pyedflib.EdfReader")
    def test_read_edf_signals_more_than_three_channels(
        self, mock_edf_reader, sample_parameters
    ):
    ...
    def test_process_single_channel(self, sample_parameters):
        ...
        num_bins = (
            int(sample_parameters["freq_limit"] / sample_parameters["freq_bin_size"])
            + 1
        )
...
class TestIntegration:
    ...
    def test_full_pipeline_integration(
        self, mock_read_signals, temp_directory, sample_parameters
    ):
