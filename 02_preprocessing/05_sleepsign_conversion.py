...
def process_all_conversions(datasets):
    ...
    for ii, (idx, dataset) in enumerate(datasets.iterrows()):
        print(
            "{} ({}/{})".format(
                dataset["file_path_sleepsign_state_annotation"], ii + 1, len(datasets)
            )
        )
...
def convert_sleepsign_hypnogram(
    old_file_path, new_file_path, sleepsign_key, epoch_duration=4
):
    """
    ...
    """
    states, intervals = load_sleepsign_hypnogram(
        old_file_path, epoch_duration=epoch_duration, mapping=sleepsign_key
    )
    export_hypnogram(new_file_path, states, intervals)


def load_sleepsign_hypnogram(
    file_path, epoch_duration=1, mapping=None, max_duration=86400, *args, **kwargs
):
    """
...
def read_sleepsign_file(file_path):
    ...
    if (
        len(empty_lines) <= 2
    ):  # only a linebreak at the end of the header and at the end of the file
        return lines
    else:
        warnings.warn(
            "{} seems to contain multiple sets of annotations. Reading only the last set.".format(
                file_path
            )
        )
...
def parse_sleepsign_data(lines, *args, **kwargs):
    ...
    *args, **kwargs
    ...
    """
    dtype = [("EpochNo", int), ("Stage", "|S2"), ("DateTime", "|S19")]
