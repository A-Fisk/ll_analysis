...
def setup_signal_configuration():
    ...
    return {
        ...
        "state_annotation_signal_labels": ["frontal EEG", "occipital EEG", "EMG"],
        "state_annotation_signal_frequency_bands": [
            (0.5, 30.0),  # Frontal EEG
            (0.5, 30.0),  # Occipital EEG
            (10.0, 45.0),  # EMG
        ],
        "time_resolution": 1,
    }
...
def setup_state_configuration():
    ...
    int_to_state = {
        ii: state for state, ii in state_to_int.items() if state != "sleep movement"
    }
    ...
    return {
        ...
        "default_view_length": 60.0,
        "default_selection_length": 4.0,
    }
...
def plot_raw_signals(raw_signals, frequency_bands, sampling_frequency, *args, **kwargs):
    ...
    *args, **kwargs
    ...
    return plot_signals(
        filtered, sampling_frequency=sampling_frequency, *args, **kwargs
    )
...
def chebychev_bandpass_filter(data, lowcut, highcut, fs, rs=60, order=5, axis=-1):
    ...
    padded = pad_along_axis(
        data, before=pad_length, after=pad_length, axis=axis, mode="reflect"
    )

    filtered = sosfilt(chebychev, padded, axis=axis)

    return remove_padding_along_axis(
        filtered, before=pad_length, after=pad_length, axis=axis
    )
